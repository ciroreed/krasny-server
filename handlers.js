var CrudHandler = function () {
    ...
    INSTANCE.readModel = function (req, res) {
        INSTANCE.serverInstance.mapper.read({
            ...
            if (res.locals.contextModel.crud && res.locals.contextModel.crud.GET) {
                res.json(filterResult(result, res.locals.contextModel.crud.GET,
    ...
    INSTANCE.createModel = function (req, res) {
        var sendResponse = function (err) {
            if (err) {
                ...
                INSTANCE.serverInstance.mapper.getLastCreation(
                    { entity: req.params.model },
                    function (err, lastModel) {
                        res.json(lastModel);
                    }
                )
            }
        };
        if (res.locals.contextModel.crud && res.locals.contextModel.crud.POST) {
            req.body = res.locals.contextModel.crud.POST(req.body,
                res.locals.authModel);
        }
        if (!req.body) {
    ...
    INSTANCE.updateModel = function (req, res) {
        if (res.locals.contextModel.crud && res.locals.contextModel.crud.PUT) {
            req.body = res.locals.contextModel.crud.PUT(req.body,
                res.locals.authModel);
        }
